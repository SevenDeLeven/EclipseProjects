...
public class EntityItem extends Entity {
	...
	public EntityItem(ItemStack stack, World world, float x, float y, long id) {
		super(world, x, y, id, new AABB(-0.4f, -0.4f, 0.4f, 0.4f));
		this.stack = stack;
		this.img = Register.getItem(stack.getItem()).getImage();
	}
	
	@Override
	public void update() {
		Item item = Register.getItem(stack.getItem());
		if (stack.getSize() != item.getMaxStackSize() && !this.isRemoved()) {
			List<Entity> items = world.getEntities(EntityItem.class);
